...
# def linearSVC(X, y):
# 	model = SVC(kernel='polynomial', probability=True)
# 	model.fit(X, y)
# 	return model
...
def train_model(df_data, target, var_cols, kernel):
	model_data = df_data.loc[:, var_cols]
	model_data = model_data.to_numpy()
	k = kernel
	trained_model = SVC(kernel=k, probability=True)
	trained_model.fit(model_data, target)
	return trained_model
